...
class GuiItemStackConfig {
   ...
   public var flag: ArrayList<String> = ArrayList()
    ...
    fun build(handler: Any, map: Map<String, String?>?): ItemStackConfig {
        ...
        if (map != null) for ((key, value) in map) {
            val foo = "$$key"
            ...
            if (skin != null) skin = PlaceholderManager.replace(handler, skin!!.replace(foo, value!!))

            lore = lore.stream().map { s: String -> s.replace(foo, value!!) }.collect(Collectors.toList()) as ArrayList<String>

            enchant = enchant.stream().map { s: String -> s.replace(foo, value!!) }.collect(Collectors.toList()) as ArrayList<String>

            flag = flag.stream().map { s: String -> s.replace(foo, value!!) }.collect(Collectors.toList()) as ArrayList<String>
        }
